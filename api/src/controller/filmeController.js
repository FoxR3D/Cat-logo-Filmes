...
server.post('/filme', async (req, resp) => {
    try
    {
        ...
        if(filme.avaliacao == undefined || filme.avaliacao <= 0) throw new Error('Avaliação do filme é obrigatória!');
        if(!filme.lancamento) throw new Error('Lançamento do filme é obrigatório!');
        if(!filme.disponivel) throw new Error('Campo Disponível é obrigatório!');
        if(!filme.nome) throw new Error('Usuário não logado!');
